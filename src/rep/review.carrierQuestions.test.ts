...
let fail = 0
const check = (ok: boolean, note: string) => { if (!ok) fail++; console.log(`${ok ? "PASS" : "FAIL"}  ${note}`) }
...
for (const [q, slug] of [[Q_WORK,"usWorkAuth"],[Q_RESIDENT,"nlgLegalResidentUs"],[Q_NY,"nlgSolicitNewYork"],[Q_FINRA,"nlgFinraLicensed"]] as const) {
  const m = carrierQuestionForLabel(q, {})
  check(!!m && m.slug === slug && m.required && m.answer === null,
    `unanswered "${q.slice(0,42)}..." → required + answer=null (blocks signing)`)
}
