...
const MAX_CONCURRENT_BROWSERS = Math.max(
  1,
  Number(process.env.SURELC_MAX_BROWSERS ?? 3),
)
/** Hard ceiling on how long one browser may hold a slot before we reclaim it. */
const BROWSER_SLOT_TTL_MS = Math.max(
  60_000,
  Number(process.env.SURELC_BROWSER_TTL_MS ?? 20 * 60_000),
)
/** How long a caller waits for a free slot before giving up. */
const SLOT_WAIT_TIMEOUT_MS = Math.max(
  60_000,
  Number(process.env.SURELC_SLOT_WAIT_MS ?? 15 * 60_000),
)
...
async function acquireSlot(logger?: pino.Logger): Promise<void> {
  if (activeBrowsers < MAX_CONCURRENT_BROWSERS) {
    activeBrowsers++
    return
  }
  logger?.info(
    { active: activeBrowsers, max: MAX_CONCURRENT_BROWSERS, queued: waiters.length },
    "browser pool full — queueing",
  )
  await new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `Timed out after ${Math.round(SLOT_WAIT_TIMEOUT_MS / 1000)}s waiting for a free browser slot (max ${MAX_CONCURRENT_BROWSERS})`,
        ),
      )
...
export async function launchChromium(logger?: pino.Logger): Promise<Browser> {
  ...
  try {
    ...
    if (!launched) releaseSlot()
  }
}

/** Release the slot when this browser goes away, with a watchdog backstop. */
function bindSlotToBrowser(browser: Browser, logger?: pino.Logger): void {
  let released = false
  ...
  const watchdog = setTimeout(() => {
    logger?.warn(
      { ttlMs: BROWSER_SLOT_TTL_MS, ...browserPoolStats() },
      "browser exceeded slot TTL — force-closing and reclaiming its slot",
    )
...
async function launchChromiumUnpooled(logger?: pino.Logger): Promise<Browser> {
