 ...
 */
import { snapshot, settle } from "../tabs/helpers.js";
const MAX_STEPS = 12;
export async function fillMiscWizard(ctx, input) {
    ...
    const miscPutResponses = [];
    const onResponse = (resp) => {
        try {
            const url = resp.url();
            if (url.includes(`/appointments-requests/${input.appointmentRequestId}/miscellaneous`) &&
                resp.request().method() === "PUT") {
                miscPutResponses.push({ status: resp.status(), url });
                logger.info({ status: resp.status(), url }, "[fillMiscWizard] misc PUT response observed");
            }
        }
        catch {
            /* ignore */
    ...
    for (let step = 1; step <= MAX_STEPS; step++) {
        ...
        if (stepName.includes("document") &&
            ...
            logger.info({ stepName }, "[fillMiscWizard] reached Documents step — stopping before Process");
            break;
        }
        // Carrier Questions step — fill misc + ComplianceDetails questions.
        // SureLC's misc step name varies by template; match loosely.
        const isMiscStep = stepName.includes("carrier questions") ||
        ...
        if (!isMiscStep) {
            ...
            if (opportunistic > 0) {
                ...
                logger.info({ opportunistic, stepName }, "[fillMiscWizard] opportunistic misc fill on non-misc step");
            }
        }
        ...
        if (!clicked) {
            ...
            logger.info({ stepName }, "[fillMiscWizard] no Next button — stopping (will not click Process)");
            break;
        }
 ...
 */
async function fillMiscRadios(ctx, answers) {
    const { page } = ctx;
    ...
    for (let idx = 0; idx < groupCount; idx++) {
        const click = await page.evaluate((args) => {
            const groups = Array.from(document.querySelectorAll("mat-radio-group"));
            ...
            const inp = g.querySelector(`input[type="radio"][value="${targetVal}"]`);
            if (!inp)
                return { skip: true, reason: "no-input", value: targetVal };
            const host = (inp.closest("mat-radio-button") || inp);
        ...
        if (!click)
            continue;
        if (click.skip) {
            ctx.logger.info({ idx, reason: click.reason, value: click.value }, "[fillMiscWizard] skipping radio group");
            continue;
        }
        ...
        try {
            ...
            ctx.logger.info({ idx, x: click.x, y: click.y, value: click.value, label: click.label }, "[fillMiscWizard] radio clicked");
            await page.waitForTimeout(250);
            clicked++;
        }
        catch (err) {
            ctx.logger.warn({ err: err?.message, label: click.label }, "[fillMiscWizard] radio click failed");
        }
    }
    return clicked;
}
 ...
 * actually filled.
 */
async function fillMiscTextInputs(ctx, textValues) {
    const { page } = ctx;
    ...
    const candidates = await page.evaluate((map) => {
        const out = [];
        ...
        const inputs = Array.from(document.querySelectorAll("input[formcontrolname], input[name]"));
        for (const inp of inputs) {
            ...
            const isDatePicker = inp.hasAttribute("matdatepicker") ||
        ...
        const matSelects = Array.from(document.querySelectorAll("mat-select[formcontrolname], mat-select[name]"));
        for (const sel of matSelects) {
            const name = sel.getAttribute("formcontrolname") || sel.getAttribute("name") || "";
            if (!name)
                continue;
            if (!(name in map))
                continue;
    ...
    for (const c of candidates) {
        try {
            ...
            if (c.kind === "text") {
                ...
                const opt = page.locator(`mat-option:has-text("${c.value}"), .mat-mdc-option:has-text("${c.value}")`);
                ...
                if (optCount > 0) {
                    ...
                    ctx.logger.warn({ name: c.name, value: c.value }, "[fillMiscWizard] mat-select option not found");
                    continue;
                }
            }
            await page.waitForTimeout(200);
            ctx.logger.info({ kind: c.kind, name: c.name, value: c.value.slice(0, 40) }, "[fillMiscWizard] text/select field filled");
            filled++;
        }
        catch (err) {
            ctx.logger.warn({ err: err?.message, name: c.name, kind: c.kind }, "[fillMiscWizard] text/select field fill failed");
        }
    }
    return filled;
}
...
async function selectFirstProductChip(ctx) {
    const result = await ctx.page.evaluate(() => {
        const chips = Array.from(document.querySelectorAll("mat-chip-option, mat-chip"));
        const fixed = chips.find((c) => /Fixed\s*Life/i.test(c.textContent || ""));
        const target = fixed || chips[0];
        if (!target)
            return { ok: false, reason: "no-chips" };
        const btn = target.querySelector("button[matchipaction], button.mdc-evolution-chip__action");
        if (!btn)
            return { ok: false, reason: "no-chip-action-button" };
