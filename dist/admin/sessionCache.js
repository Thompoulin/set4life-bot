...
const SESSION_CACHE_DISABLED = process.env.BOT_DISABLE_SESSION_CACHE === "1" ||
 ...
 */
export async function getAuthenticatedPage(browser, adminCreds, logger, opts = {}) {
    ...
    if (!SESSION_CACHE_DISABLED &&
        ...
        try {
            ...
            logger.warn({ err: err?.message }, "admin session: reuse path errored — falling back to full login");
            invalidateSession();
            if (context)
                await context.close().catch(() => undefined);
        }
    ...
    if (loginResult.ok && !SESSION_CACHE_DISABLED) {
        try {
            ...
            logger.info({ expiresInMs: cached.expiresAt - Date.now() }, "admin session: cached authenticated storageState for reuse");
        }
        catch (err) {
            ...
            logger.warn({ err: err?.message }, "admin session: failed to snapshot storageState (login still valid)");
        }
    }
 ...
 */
export async function tryReuseContext(browser, email, logger, opts = {}) {
    const snapshot = cached;
    if (SESSION_CACHE_DISABLED ||
        !snapshot ||
 ...
 */
async function verifyAuthenticated(page, logger) {
    try {
...
async function hasBgaToken(page) {
    return page
        .evaluate(() => {
...
function readJwtExpiryMs(value) {
