 ...
 */
import { settle, snapshot, } from "../tabs/helpers.js";
export async function processBgaReview(ctx, input) {
    const { page, logger } = ctx;
    if (!input.requestId) {
        return {
            ok: false,
            reason: "appointmentRequestId missing — caller must extract from rep-review email URL",
        };
    }
    ...
    for (let step = 1; step <= MAX_STEPS; step++) {
        ...
        if (finalBtn) {
            ...
            const dialogText = await page.evaluate(() => Array.from(document.querySelectorAll("mat-dialog-container, .mat-mdc-dialog-container, .cdk-overlay-pane"))
...
async function fillRecruiterHierarchy(ctx, recruiterName) {
    const { page, logger } = ctx;
    // Detect partial fill (Code cell still has ag-cell--invalid).
    const partial = await page.evaluate(() => {
        const labels = Array.from(document.querySelectorAll('[role="gridcell"][col-id="label"]'));
    ...
    if (partial) {
        const reset = await page.evaluate(() => {
            ...
            const reset = btns.find((b) => /reset to default/i.test((b.textContent || "").trim()));
            if (reset) {
                ;
                reset.click();
                return true;
    ...
    logger.info({ x: cellInfo.x, y: cellInfo.y }, "[Phase C] dblclick Recruiter Name cell");
    ...
    const action = await page.evaluate(() => {
        const btns = Array.from(document.querySelectorAll("mat-dialog-container button, .mat-mdc-dialog-container button, .cdk-overlay-pane button"));
        const useInHierarchy = btns.find((b) => !b.disabled &&
        ...
        const apply = btns.find((b) => !b.disabled &&
            !b.classList.contains("mat-mdc-button-disabled") &&
 ...
 *  override provided. */
async function fillCarrierAgencyAnswers(ctx, answers) {
    const { page } = ctx;
    await page.evaluate((map) => {
        const groups = Array.from(document.querySelectorAll("mat-radio-group"));
        for (const g of groups) {
            const groupName = g.querySelector('input[type="radio"]')?.name;
            if (!groupName)
                continue;
            const labelEl = g
            ...
            const target = g.querySelector(`input[type="radio"][value="${value || "N"}"]`);
...
async function findFinalActionButton(ctx) {
    return ctx.page.evaluate(() => {
        const btns = Array.from(document.querySelectorAll("button"));
        const candidates = btns.filter((b) => !b.disabled &&
            !b.classList.contains("mat-mdc-button-disabled") &&
            b.offsetParent !== null);
        for (const b of candidates) {
            const text = (b.textContent || "").replace(/\s+/g, " ").trim();
            if (/^(?:Process|Submit Request|Submit|Approve|Send|Complete|Finish)\b/i.test(text)) {
                return text;
            }
        }
        return null;
...
async function clickButtonByText(ctx, label) {
