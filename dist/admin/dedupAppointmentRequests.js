...
export async function dedupAppointmentRequests(page, producerId, gaId, logger) {
    ...
    if (!bearer) {
        logger.warn({ producerId }, "[dedup] could not capture Bearer JWT — skipping dedup");
        return null;
    }
    ...
    if (!res.ok) {
        logger.warn({ producerId, status: res.status }, "[dedup] could not list appointment-requests");
        return null;
    }
    ...
    const active = Array.from(byCarrier.values()).reduce((n, list) => n + Math.min(1, list.length), 0);
    return {
        total: rows.length,
        active,
