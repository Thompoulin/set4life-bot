 ...
 */
import { firstVisible, fillByLabel, fillForce, fillIfEmpty, gotoBga, selectByLabel, uploadRemoteFile, settle, snapshot, } from "../tabs/helpers.js";
const baseUrl = (id) => `https://surelc.surancebay.com/bga/producers/${id}`;
async function downloadRemoteFile(remoteUrl, prefix) {
...
function parseMoneyLimit(text, patterns) {
...
function extractEoDataFromText(rawText) {
    ...
    const combined = text.match(new RegExp(`policy\\s*(?:number|no\\.?|#)[\\s\\S]{0,120}?(\\d{6,12}?)(${VALID_DATE})(?:[\\s\\S]{0,15}?(${VALID_DATE}))?`, "i")) ||
        text.match(new RegExp(`certificate\\s*(?:number|no\\.?|#)[\\s\\S]{0,120}?(\\d{6,12}?)(${VALID_DATE})(?:[\\s\\S]{0,15}?(${VALID_DATE}))?`, "i"));
    ...
    const biberkPolicy = !combined?.[1] &&
        (text.match(new RegExp(`(?:claims[-\\s]made|errors\\s*&\\s*omissions|professional\\s*liability)[\\s\\S]{0,80}?\\b${POLICY_TOKEN}\\b`, "i"))?.[1] ||
            text.match(new RegExp(`\\b${POLICY_TOKEN}\\b\\s*(?:professional\\s*liability|errors\\s*&\\s*omissions|claims[-\\s]made)`, "i"))?.[1]);
    ...
    const consecutivePair = text.match(new RegExp(`(${VALID_DATE})\\s*[\\/\\s]?\\s*(${VALID_DATE})`, "i"));
    let pairEffective;
    let pairExpiration;
    if (consecutivePair) {
    ...
    const effectiveDate = parseDateValue(dateFromCombined) ||
        ...
        pairEffective;
    const expirationDate = parseDateValue(expirationFromCombined) ||
    ...
    // aggregate context.
    const dualLimit = text.match(/(?:per\s*(?:claim|occurrence)|each\s*(?:claim|occurrence))[\s\S]{0,40}?aggregate[\s\S]{0,150}?(\$\s*\d[\d,]*(?:\.\d+)?)\s*\/?\s*(\$\s*\d[\d,]*(?:\.\d+)?)/i);
    const caseLimit = parseMoneyLimit(text, [
    ...
    const totalLimit = parseMoneyLimit(text, [
...
async function extractEoDataFromPdf(buffer, logger) {
    try {
 ...
 */
async function describeUploadablePage(page) {
...
async function isSureLcLoginOrOAuthPage(page) {
    const url = page.url();
    try {
        const u = new URL(url);
        if (/accounts\.surancebay\.com$/i.test(u.hostname))
            return true;
        if (/\/oauth\b/i.test(u.pathname))
            return true;
    }
    catch {
    ...
    const visibleButtons = await page
        .$$eval("button", (els) => els
        .filter((b) => b.offsetParent !== null)
        .map((b) => b.innerText.trim().toUpperCase()))
        .catch(() => []);
    return visibleButtons.includes("LOGIN") && visibleButtons.some((b) => b.includes("FORGOT PASSWORD"));
}
async function assertOnProducerTab(page, producerId, slug) {
 ...
 */
async function enterProducerProfile(ctx, producerId) {
    ...
    const navList = await gotoBga(page, "https://surelc.surancebay.com/bga/producers", logger);
    if (!navList.ok) {
    ...
    const nameLink = (await page.$(`[role="row"][row-id="${producerId}"] [col-id="name"] sb-text-link span.main-text`)) ||
        (await page.$(`[role="row"][row-id="${producerId}"] [col-id="name"] bga-producer-name-render`));
    if (!nameLink) {
...
export async function fillFullProfile(ctx, input) {
    ...
    const tabs = {};
    const runIsolated = async (name, fn) => {
    ...
    tabs.profile = await runIsolated("profile", () => fillProfileTab(ctx, input.producerId, input.residentAddress));
    tabs.dba = await runIsolated("dba", () => fillDba(ctx, input.producerId, input.dba));
    tabs.questions = await runIsolated("questions", () => fillQuestions(ctx, input.producerId, input.questions));
    tabs.finra = await runIsolated("finra", () => fillFinra(ctx, input.producerId, input.questions));
    tabs.training = await runIsolated("training", () => fillTraining(ctx, input.producerId, input.training));
    tabs.eno = await runIsolated("eno", () => fillEno(ctx, input.producerId, input.eno));
    tabs.signature = await runIsolated("signature", () => fillSignature(ctx, input.producerId, input.signature));
    return tabs;
}
// ─── FINRA ────────────────────────────────────────────────────────────
...
async function fillFinra(ctx, producerId, input) {
    const { page, logger } = ctx;
    ...
    const buttons = await page.$$(`mat-radio-button:has(input[value="${wantValue}"])`);
    let clicked = 0;
...
async function fillProfileTab(ctx, producerId, input) {
    const { page, logger } = ctx;
    ...
    const hasInvalidAlertPre = await page
        .$$eval("body", (els) => /Resident address is not valid/i.test(els[0]?.innerText || ""))
        .catch(() => false);
    ...
    const residenceEditBtn = await page
        .evaluateHandle(() => {
        const items = Array.from(document.querySelectorAll("sb-producer-profile-address-item"));
        const residenceCard = items.find((it) => /Residence/i.test(it.innerText || ""));
        if (!residenceCard)
            return null;
        return residenceCard.querySelector('button[mattooltip="Edit Address"]');
    })
        .catch(() => null);
    ...
    if (useBusinessOrMailing) {
        try {
            ...
            await useBusinessOrMailing.click();
    ...
    if (stillInvalid) {
        return {
            ok: false,
            reason: "Resident address still flagged invalid after dialog save — USPS validator may have rejected the address (typo, apt# missing, non-deliverable). Manual review needed.",
...
// ─── DBA ──────────────────────────────────────────────────────────────
async function fillDba(ctx, producerId, input) {
    const { page } = ctx;
    ...
    if (input.businessType && input.businessType !== "License Only") {
        console.warn(`[SureLC bot fillProfile] businessType=${input.businessType} ignored — forcing "License Only" per owner directive`);
    }
    await selectByLabel(page, "How will this producer do business", "License Only").catch(() => false);
    ...
    if (solicit) {
        try {
            const tag = await solicit.evaluate((el) => el.tagName);
            if (tag === "SELECT") {
                await solicit.selectOption({ label: input.solicitingFor });
            }
            else {
                await solicit.fill(input.solicitingFor);
...
async function fillQuestions(ctx, producerId, input) {
    ...
    const parentYes = new Set();
    if (input?.surelcAnswers) {
        for (const [slug, ans] of Object.entries(input.surelcAnswers)) {
            if (ans.answer === "yes") {
                const parent = SUB_TO_PARENT[slug];
                if (parent)
                    parentYes.add(parent);
            }
        }
    }
    ...
    for (const parentNum of parentYes) {
        try {
            ...
            const yesRadio = await page.$(`tr:has-text("${parentNum}.") input[type="radio"][value="yes" i]`);
            if (!yesRadio) {
                // Fallback — match by leading number text + Yes label.
                const fallback = await page.$(`*:has(text="${parentNum}") >> input[type="radio"][value="yes" i]`);
                if (fallback) {
    ...
    if (input?.surelcAnswers) {
        for (const [slug, ans] of Object.entries(input.surelcAnswers)) {
            ...
            const occurrenceDate = ans?.occurrenceDate;
            if (occurrenceDate) {
                ...
                const mmddyyyy = isoMatch
                    ...
                    : occurrenceDate;
                try {
                    const dateInput = await page.$(`tr:has-text("${parentNum}.") + tr input[placeholder*="MM/DD" i], ` +
                    ...
                    if (dateInput) {
                        ...
                        await dateInput.dispatchEvent("change").catch(() => false);
                        await dateInput.dispatchEvent("blur").catch(() => false);
            ...
            if (!ans.documents?.length)
                continue;
            // Find the row's expanded explanation area.
            const explanationArea = await page.$(`tr:has-text("${parentNum}.") + tr [class*="explanation"], ` +
            ...
            const expDoc = (ans.documents || []).find((d) => d?.kind === "explanation_letter" && d?.explanation?.explanation);
            const explanationText = expDoc?.explanation?.explanation ||
                input?.explanations?.[slug];
            if (explanationText && explanationArea) {
                try {
                    await explanationArea.fill(explanationText);
            ...
            for (let i = 0; i < ans.documents.length; i++) {
                ...
                try {
                    ...
                    if (!res.ok)
                        continue;
                    const buf = Buffer.from(await res.arrayBuffer());
                    const localPath = path.join(os.tmpdir(), `surelc-q${parentNum}-${Date.now()}-${doc.fileName || "doc.pdf"}`);
                    await fs.writeFile(localPath, buf);
                    // Try filechooser first — find the UPLOAD button scoped to
                    // the question row (or in the explanation area below it).
                    const uploadBtn = await page.$(`tr:has-text("${parentNum}.") + tr button:has-text("UPLOAD"), ` +
                    ...
                    if (!attached) {
                        const fileInputs = await page.$$(`tr:has-text("${parentNum}.") + tr input[type="file"]`);
                        if (fileInputs[i]) {
                            await fileInputs[i].setInputFiles(localPath);
                            attached = true;
                        }
                    }
                    if (!attached) {
    ...
    if (input?.finraRegistered) {
        const yes = await page.$('label:has-text("FINRA") input[type="radio"][value="yes" i]');
        if (yes)
            await yes.check({ force: true }).catch(() => false);
        if (input.finraCrd) {
            await fillByLabel(page, "CRD", input.finraCrd).catch(() => false);
        }
    }
    else {
        const no = await page.$('label:has-text("FINRA") input[type="radio"][value="no" i]');
        if (no)
            await no.check({ force: true }).catch(() => false);
    }
    // ── 3. Military status.
    ...
    if (input?.attestationName) {
        await fillByLabel(page, "Attestation Name", input.attestationName)
            .catch(() => false)
            .then((ok) => ok ? true : fillByLabel(page, "Full Name", input.attestationName))
            .catch(() => false);
    }
    if (input?.attestationDate) {
        await fillByLabel(page, "Attestation Date", input.attestationDate)
            .catch(() => false)
            .then((ok) => ok ? true : fillByLabel(page, "Date Signed", input.attestationDate))
            .catch(() => false);
    }
    if (input?.attestationInitials) {
        await fillByLabel(page, "Initials", input.attestationInitials).catch(() => false);
    }
    // ── 5. Employment + address history — repeating rows TODO.
    if ((input?.employmentHistory?.length ?? 0) > 0) {
        logger.warn({ rows: input?.employmentHistory?.length }, "employment history not implemented (repeating-row UI selectors needed)");
    }
    if ((input?.addressHistory?.length ?? 0) > 0) {
        logger.warn({ rows: input?.addressHistory?.length }, "address history not implemented (repeating-row UI selectors needed)");
    }
 ...
 */
async function findSubQuestionRadio(page, slug, questionText, answer) {
    const candidates = [
    ...
    if (questionText) {
        ...
        const el = await page.$(`*:has-text("${key}") >> input[type="radio"][value="${answer}" i]`);
        if (el)
            return el;
    }
    return null;
}
async function findFieldNearSlug(page, slug, selectors) {
...
async function findFileInputsNearSlug(page, slug) {
    const handles = await page.$$(`[data-slug="${slug}"] input[type="file"], [data-question="${slug}"] input[type="file"], [id*="${slug}"] input[type="file"]`);
    const out = [];
    for (const h of handles) {
        const attrs = await h.evaluate((el) => Array.from(el.attributes)
...
// ─── Training (AML) ───────────────────────────────────────────────────
async function fillTraining(ctx, producerId, input) {
    const { page, logger } = ctx;
    ...
    for (let i = 0; i < 3; i++) {
        const onAddSubpage = await page
            .$$eval("body", (els) => /Add Manually|Add Training|Select or Upload Certificate/i.test(els[0]?.innerText || ""))
            .catch(() => false);
        if (!onAddSubpage)
            break;
    ...
    if (amlUploadBtn) {
        try {
            ...
            if (res.ok) {
                ...
                const fs = await import("node:fs/promises");
                const os = await import("node:os");
                const localPath = path.join(os.tmpdir(), `aml-${Date.now()}.pdf`);
                await fs.writeFile(localPath, buf);
    ...
    if (!uploaded) {
        await assertOnProducerTab(page, producerId, "training");
        uploaded = await uploadRemoteFile(page, 'input[type="file"]', input.amlCertificateUrl, logger).catch(() => false);
    }
    ...
    while (Date.now() - start < PARSE_BUDGET_MS) {
        ...
        const stillProcessing = await page
            .$$eval("body", (els) => /Recognition|Processing|Uploading|Loading document/i.test(els[0]?.innerText || ""))
            .catch(() => false);
        if (!stillProcessing) {
            cleared = await waitForTabClear(page, "training", POLL_MS);
            break;
    ...
    if (courseOption) {
        ...
        await courseOption.click().catch(() => undefined);
        await settle(page, 600);
    }
    if (input.amlCompletionDate) {
        ...
        if (isoMatch)
            mmddyyyy = `${isoMatch[2]}/${isoMatch[3]}/${isoMatch[1]}`;
 ...
 */
async function fillEnoDate(page, labels, value, logger) {
    if (!value)
        return false;
    ...
    for (const label of labels) {
        const result = await page.evaluate(({ labelText, mmddYyyy, iso }) => {
            ...
            if (!ff)
                return { ok: false, reason: "no-form-field-ancestor" };
            ...
            const hidden = ff.querySelector('input.mat-datepicker-input');
            if (!visible || !hidden)
...
async function fillEno(ctx, producerId, input) {
    const { page, logger } = ctx;
    ...
    for (let i = 0; i < 2; i++) {
        const onAddForm = await page
            .$$eval("body", (els) => /Add Individual E&O Policy|Add New Certificate|Data is not saved/i.test(els[0]?.innerText || ""))
            .catch(() => false);
        if (!onAddForm)
            break;
    ...
    if (existingRows.length > 0) {
        logger.info({ count: existingRows.length }, "[E&O] producer already has policy row(s) — skipping ADD");
    ...
    if (missing.length > 0) {
        ...
        return {
            ok: false,
            reason: `E&O PDF could not be read well enough. Missing ${missing.join(", ")}. ` +
                `Do not guess these values. ${await describeUploadablePage(page)}`,
            details: { extracted, missing },
        };
    }
    const policyNumberValue = policyNumber;
    const carrierValue = carrier;
    const caseLimitValue = caseLimit;
    const totalLimitValue = totalLimit;
    ...
    while (Date.now() - start < PARSE_BUDGET_MS) {
        ...
        const bodyText = await page
            .$$eval("body", (els) => els[0]?.innerText || "")
            .catch(() => "");
        const stillWorking = /Processing\.\.\./i.test(bodyText) ||
    ...
    const limitsAfterFill = await page
        ...
        const readByLabel = (target) => {
            const m = labels.find((l) => (l.textContent || "")
                .trim()
                .toLowerCase()
                .replace(/\*$/, "")
        ...
        return {
            ...
            totalLimit: readByLabel("Total Limit") ?? readByLabel("Aggregate"),
        };
    })
        .catch(() => ({ caseLimit: null, totalLimit: null }));
    logger.info({ rendered: limitsAfterFill, intended: { caseLimitValue, totalLimitValue } }, "[E&O] post-fill rendered limits");
    const renderedCase = limitsAfterFill.caseLimit ?? 0;
    const renderedTotal = limitsAfterFill.totalLimit ?? 0;
    if (renderedCase > 0 && renderedTotal > 0 && renderedTotal < renderedCase) {
        logger.warn({ renderedCase, renderedTotal }, "[E&O] Total < Case after fill — force-equalizing Total up to Case to unblock SAVE");
 ...
 * matcher.
 */
async function fillSignature(ctx, producerId, input) {
    const { page, logger } = ctx;
    ...
    if (hasUploaded) {
        ...
        if (input?.signatureImageUrl) {
            try {
                const overwriteResult = await pushSignatureViaApi(page, producerId, fileUrl, input.signatureImageUrl, logger);
                if (overwriteResult.ok) {
                    await snapshot(ctx, "tab-signature-02-api-overwrite");
                    logger.info({ formId: overwriteResult.formId }, "[Signature] API push overwrote existing signature without REMOVE");
                    return {
                        ok: true,
                        details: {
                            autoSaved: true,
        ...
        try {
            await page.waitForSelector('mat-dialog-container, [role="dialog"]', { timeout: 8_000 });
            const affirm = (await page.$('mat-dialog-container button:has-text("YES")')) ||
            ...
            if (!affirm) {
                ...
                const dialogText = await page
                    .$$eval("mat-dialog-container, [role='dialog']", (els) => els.map((e) => (e.textContent || "").trim()).join(" | "))
                    .catch(() => "");
    ...
    try {
        ...
        if (!result.ok)
            return { ok: false, reason: result.reason || "API push failed" };
 ...
 */
async function pushSignatureViaApi(page, producerId, pdfUrl, signatureImageUrl, logger) {
    ...
    if (!bearer)
        return { ok: false, reason: "no BGA bearer captured from SPA" };
    // Download the signature-auth PDF + the bare drawn-PNG.
    const pdfRes = await fetch(pdfUrl);
    if (!pdfRes.ok)
        return { ok: false, reason: `fetch pdfUrl failed: HTTP ${pdfRes.status}` };
    const pdfBuf = Buffer.from(await pdfRes.arrayBuffer());
    const pngRes = await fetch(signatureImageUrl);
    if (!pngRes.ok)
        return { ok: false, reason: `fetch pngUrl failed: HTTP ${pngRes.status}` };
 ...
 */
async function positionCropBoxOverSignatureLine(page, logger) {
    const docImg = (await page.$(".ngx-ic-source-image")) ||
    ...
    const setByDom = await page
        .evaluate((t) => {
        const cropper = document.querySelector(".ngx-ic-cropper");
        if (!cropper)
            return false;
...
// ─── Helpers ──────────────────────────────────────────────────────────
async function goToTab(page, producerId, slug, logger) {
...
async function isTabComplete(page, tabHref) {
...
async function waitForTabClear(page, tabHref, timeoutMs = 10_000) {
...
async function isTabGreen(page, label) {
