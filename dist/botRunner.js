...
export async function runActivation(input, logger) {
    ...
    try {
        ...
        if (phases.has("admin_setup")) {
            ...
            const finishAdd = await progress.startStep("phaseA_add_producer", "Adding producer or linking existing");
            ...
            if (producerId && input.contracting) {
                ...
                if (incompleteTabs.length > 0) {
                    const finishGate = await progress.startStep("phaseA_contracting", "Submitting contracting requests via Fastlane");
                    ...
                    await finishGate({
                        ok: false,
                        msg: "Contracting BLOCKED — profile is not fully filled. " +
                            `Fix these tab(s) first and re-run: ${incompleteTabs.join("; ")}.`,
                    });
                }
                else {
                    const finishContracting = await progress.startStep("phaseA_contracting", "Submitting contracting requests via Fastlane");
                    ...
                    if (!preDedupSkipsFastlane) {
                        const { runFastlaneOneProducerManyCarriers } = await import("./admin/fastlane.js");
                        ...
                        if (r.ok) {
                            ...
                            if (transferCarrierNames.length > 0) {
                                try {
                                    const { setTransferTypesForProducer } = await import("./admin/setTransferTypes.js");
                                    const flipResult = await setTransferTypesForProducer(tabCtx.page, logger, { producerId, transferCarrierNames });
                                    logger.info({
                                    ...
                                    if (flipResult.failed.length > 0) {
                                        adminPhase.contracting.failed.push(...flipResult.failed.map((f) => ({
            ...
            if (!adminPhase.ok) {
                ...
                if (adminPhase.addedProducer && !adminPhase.addedProducer.ok) {
                    reasons.push(`addProducer: ${adminPhase.addedProducer.reason || "unknown"}`);
                }
                if (adminPhase.contracting?.failed?.length) {
                    reasons.push(`contracting: ${adminPhase.contracting.failed
                ...
                if (adminPhase.profile) {
                    ...
                    if (failedTabs.length)
                        reasons.push(`tabs: ${failedTabs.join("; ")}`);
                }
                result.needsHumanReason =
        ...
        if (phases.has("rep_review") && input.repReview) {
            const finishRep = await progress.startStep("phaseB_rep_review", "Rep auth (last 6 SSN + DOB) + signing each carrier");
            try {
                ...
                results: [
                    {
                        carrier: "(disabled)",
                        ok: true,
                        reason: "Phase C disabled by owner directive — admin processes BGA→Carrier manually",
                    },
                ],
            };
            await progress.report({
                ...
                status: "info",
                message: "Phase C skipped by policy — admin will release At-BGA appointments manually",
            });
        }
        if (result.phases.admin_setup?.ok &&
...
function finalize(result, evidenceDir, files) {
