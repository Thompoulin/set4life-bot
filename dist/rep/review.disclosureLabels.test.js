...
const L_LICENSE = "Have you ever had an insurance or securities license denied, suspended, cancelled or revoked?*";
const L_INVESTIGATION = "Are you currently under investigation by any legal or regulatory authority?*";
const L_EO = "Has any lawsuit or claim ever been made against your surety company, or errors and omissions insurer, arising out of your sales or practices, or, have you been refused surety?";
// Present on the same wizards, must stay unmatched by the widened patterns.
const L_CHARGEBACK = "Does any insurer, insured, or other person claim any commission chargeback or other indebtedness from you as a result of any insurance transactions or business?*";
...
let failures = 0;
function expectYn(label, disc, want, note) {
...
expectKeys(L_LICENSE, { q4_license_denied: false, q5_license_revoked: true }, ["q5_license_revoked"], "combined licence question places q5");
expectKeys(L_LICENSE, { q4_license_denied: true, q5_license_revoked: true }, ["q4_license_denied", "q5_license_revoked"], "combined licence question places BOTH q4 and q5 (the audit bug)");
expectYn(L_LICENSE, { q5_license_revoked: true }, "Y", "licence question answers Yes on q5");
expectYn(L_LICENSE, { q4_license_denied: false, q5_license_revoked: false }, "N", "clean rep still answers No");
// ── 2. "under investigation" phrasing (no 'fraud', no 'subject of') ──
expectKeys(L_INVESTIGATION, { q11_fraud_investigation: true }, ["q11_fraud_investigation"], "'currently under investigation' places q11 (Jhovanny / Michael Wills)");
...
expectKeys(L_EO, { q15_eo_claim: true }, ["q15_eo_claim"], "'claim ... errors and omissions' places q15 (Sean Way)");
...
expectKeys(L_CHARGEBACK, { q15_eo_claim: true, q9_unpaid_premiums: false }, [], "chargeback question is not an E&O claim despite the word 'claim'");
...
for (const [name, l] of Object.entries({ L_LICENSE, L_INVESTIGATION, L_EO, L_CHARGEBACK })) {
    ...
    if (!ok)
        failures++;
    console.log(`${ok ? "PASS" : "FAIL"}  q13_ce_violation must NOT be placed on ${name} (wrong question)`);
}
