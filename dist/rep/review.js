 ...
 */
import { firstVisible, settle, snapshot, } from "../tabs/helpers.js";
/**
 ...
 */
const DISCLOSURE_LABEL_PATTERNS = [
...
export async function repReview(browser, input, parentLogger, jobId) {
    ...
    try {
        ...
        const matInputAppeared = await page
            .waitForSelector('input[matinput], input.mat-mdc-input-element, input[type="password"], mat-form-field input, sb-auth input, auth-component input', { timeout: 30_000 })
            .then(() => true)
            .catch(() => false);
        ...
        try {
            const inputAttrs = await page.$$eval('input', (els) => els
        ...
        for (let i = 0; i < totalIterations; i++) {
            try {
                ...
                if (i + 1 < totalIterations) {
                    ...
                    logger.info({ signed, attempted: i + 1 }, "[Rep loop] all queued carriers processed");
                }
            }
            catch (err) {
...
async function reviewOneCarrier(ctx, idx, input) {
    ...
    if (/\/appointment\/[^/]+\/reviewed/.test(url)) {
        ...
        if (input.prefillOnly) {
            ...
            logger.info({ url }, "[Rep step6] appointment already signed (auth landed on /reviewed) — skipping");
    ...
    for (let i = 0; i < 6; i++) {
        if (/\/wizard\/misc\/info/.test(page.url()))
            break;
    ...
    logger.info({ url: page.url(), step: "step4-entry" }, "[Rep prefillOnly] reached carrier-questions step");
    ...
    if (input.prefillOnly) {
        logger.info({ carrier: idx }, "[Rep prefillOnly] steps 1-5 saved; skipping Step 6 sign");
    ...
    if (!pdfReady) {
        ...
        const isWizardBlock = url.includes("/wizard/welcome") ||
    ...
    while (Date.now() < deadline) {
        signHandle = await page
            .evaluateHandle(() => {
            return (Array.from(document.querySelectorAll("button")).find((b) => !b.disabled &&
                !b.classList.contains("mat-mdc-button-disabled") &&
                /apply.*signature/i.test(b.textContent || "")) ?? null);
        })
            .catch(() => null);
    ...
    if (!clicked) {
        ...
        try {
            const visibleBtns = await page.$$eval('button', (btns) => btns
                .filter((b) => b.offsetParent !== null && !b.disabled)
                .map((b) => b.textContent?.trim().slice(0, 60)));
            logger.warn({ visibleEnabledButtons: visibleBtns }, "[Rep step6] no Apply Signature button matched");
        }
        catch {
            /* ignore */
        }
    ...
    const confirm = await page.$('text=/sent|submitted|thank you|complete|success|received|signed/i');
    if (!confirm) {
        return { ok: false, reason: "no confirmation marker after signing" };
    }
    return { ok: true };
}
async function acceptPoliciesIfShown(page) {
    ...
    for (let i = 0; i < 4; i++) {
        const onPolicies = page.url().includes("/policies") ||
        ...
        await page
            .evaluate(() => {
            const cb = document.querySelector(".acceptCheckbox input[type='checkbox'], mat-checkbox.acceptCheckbox input");
            if (cb && !cb.checked)
                cb.click();
        })
            .catch(() => undefined);
        ...
        const acceptEnabled = await page
            .waitForSelector('button.acceptPolicyButton:not([disabled]):not(.mat-mdc-button-disabled)', { timeout: 10_000 })
            .catch(() => null);
        if (!acceptEnabled) {
            // Last resort — click via JS bypassing visibility/enabled gates.
            await page
                .evaluate(() => {
                const btn = Array.from(document.querySelectorAll("button.acceptPolicyButton")).find((b) => !b.disabled);
                btn?.click();
            })
                .catch(() => undefined);
        }
 ...
 */
async function fillCarrierProfileText(ctx, profile) {
    const { page, logger } = ctx;
    if (!profile)
        return;
    ...
    for (const [fc, val] of Object.entries(map)) {
        ...
        const input = await page.$(`input[matinput][name="${fc}"], input[matinput][formcontrolname="${fc}"], mat-form-field input[name="${fc}"], mat-form-field input[formcontrolname="${fc}"]`);
        if (!input)
            continue;
        try {
    ...
    const ticked = await page.evaluate(() => {
        const cbs = Array.from(document.querySelectorAll('input[type="checkbox"][required]'));
        let count = 0;
        for (const cb of cbs) {
    ...
    const recovered = await page.evaluate(() => {
        const invalidGroups = Array.from(document.querySelectorAll("mat-radio-group.ng-pristine.ng-invalid, mat-radio-group.question__select--invalid"));
        let fixed = 0;
        for (const g of invalidGroups) {
            ...
            const noInput = g.querySelector('input[type="radio"][value="N"], input[type="radio"][value="false"]');
            if (!noInput)
                continue;
            noInput.scrollIntoView({ block: "center" });
            const host = noInput.closest("mat-radio-button");
            if (host) {
    ...
    if (recovered > 0) {
        logger.info({ recovered }, "[Rep step4] re-bound radio groups that stayed ng-pristine after first click");
    }
}
/**
 ...
 */
async function dismissReviewSignWarning(ctx) {
    const { page, logger } = ctx;
    const okBtn = await page
        .locator("mat-dialog-container button, .mat-mdc-dialog-container button, .cdk-overlay-pane button")
 ...
 */
async function clickSignConsentDialog(ctx) {
    const { page, logger } = ctx;
    ...
    while (Date.now() < deadline) {
        const handle = await page
            ...
            for (const d of Array.from(dialogs)) {
                const btn = Array.from(d.querySelectorAll("button")).find((b) => !b.disabled &&
                    !b.classList.contains("mat-mdc-button-disabled") &&
 ...
 */
async function defaultRadiosToValue(page, value) {
 ...
 */
async function tickTelemarketingOnlyIfPresent(ctx) {
    const { page, logger } = ctx;
    ...
    if (!handle)
        return; // page doesn't have this step / not present
    ...
    try {
        ...
        logger.warn({ err: err?.message }, "[Rep] tickTelemarketingOnlyIfPresent: Playwright click failed");
    }
}
async function clickNextWhenEnabled(ctx) {
    const { page } = ctx;
    ...
    await page
        .waitForSelector('button:has-text("NEXT"), button:has-text("Next"), button:has-text("Continue"), button:has-text("Submit")', { timeout: 15_000 })
        .catch(() => null);
    const clicked = await page.evaluate(() => {
        const candidates = Array.from(document.querySelectorAll("button")).filter((b) => {
            if (b.disabled)
                return false;
            if (b.classList.contains("mat-mdc-button-disabled"))
                return false;
...
async function waitForPdfViewer(page, maxRetries = 2) {
    ...
    try {
        ...
        if (/agency has withdrawn this request/i.test(earlyDiag.bodyExcerpt) ||
            /contact your agency for details/i.test(earlyDiag.bodyExcerpt)) {
            ;
            page._lastPdfDiag = { ...earlyDiag, withdrawn: true };
            return false;
        }
    }
    catch {
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            await page
                .waitForFunction(() => {
                const loader = document.querySelector("sb-page-loader, .pane__spinner-text, .container__spinner-text");
                if (!loader)
                    return true;
    ...
    try {
        const diag = await page.evaluate(() => {
            ...
            for (const el of Array.from(all)) {
                ...
                const childText = Array.from(node.children)
                    ...
                    .join("")
                    .trim();
                if (childText.length > 0 && childText.length === text.length)
                    continue;
            ...
            return {
                ...
                loaderText: sample("sb-page-loader, .pane__spinner-text, .container__spinner-text"),
                url: location.href,
                title: document.title,
                bodyExcerpt: (document.body.innerText || "").replace(/\s+/g, " ").slice(0, 300),
            };
        });
        page._lastPdfDiag = diag;
        ...
        try {
            const stepNavSelectors = 'sb-stepper button, .step-nav button, .stepper button, [class*="step-list"] button, mat-step-header';
            ...
            for (let i = 0; i < Math.min(stepCount, 8); i++) {
                try {
                    ...
                    if (i >= navHandles.length)
                        break;
                    const label = await navHandles[i].evaluate((el) => el.innerText.replace(/\s+/g, " ").trim().slice(0, 80));
                    ...
                    const content = await page.evaluate(() => {
                        ...
                        for (const el of Array.from(all).slice(0, 40)) {
                            ...
                            if (isRed)
                                redFields.push(text.slice(0, 80));
                            else if (text.length > 5)
                                allFields.push(text.slice(0, 80));
                            if (redFields.length >= 10 && allFields.length >= 15)
                                break;
                        }
...
async function scrollViewerToBottom(page) {
    await page.evaluate(() => {
