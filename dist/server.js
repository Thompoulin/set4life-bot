...
if (!BEARER) {
    logger.warn("BOT_SHARED_SECRET not set — container will run but all requests will 401");
}
...
app.post("/run-activation", async (req, res) => {
    ...
    logger.info({ jobId: input.jobId, agentOpenId: input.agentOpenId, phases: input.phases }, "starting activation");
    try {
...
app.post("/producer-appointments", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const r = await fetch(`https://surelc.surancebay.com/surecrm/appointments-requests?producerId=${producerId}&gaId=${gaId}`, { headers: { Authorization: `Bearer ${bearer}` } });
            if (!r.ok) {
                return res
                    .status(502)
...
app.post("/resend-rep-emails", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const listRes = await fetch(`https://surelc.surancebay.com/surecrm/appointments-requests?producerId=${producerId}&gaId=${gaId}`, { headers: { Authorization: `Bearer ${bearer}` } });
            if (!listRes.ok) {
            ...
            for (const r of stuck) {
                ...
                try {
                    const x = await fetch(`https://surelc.surancebay.com/surecrm/appointments-requests/${id}/email`, {
                        method: "POST",
                        headers: {
...
app.post("/create-appointment-requests", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (!state)
                return res.status(400).json({ ok: false, error: "no resident state found" });
            // Producer's existing non-Discarded carriers — skip these
            const existing = await fetch(`https://surelc.surancebay.com/surecrm/appointments-requests?producerId=${producerId}&gaId=${gaId}`, { headers: { Authorization: `Bearer ${bearer}` } }).then((r) => r.json());
            const existingCarriers = new Set((Array.isArray(existing) ? existing : [])
            ...
            const producerRecord = await fetch(`https://surelc.surancebay.com/surecrm/producer/${producerId}`, { headers: { Authorization: `Bearer ${bearer}` } })
                .then((r) => (r.ok ? r.json() : null))
                .catch(() => null);
            const producerEmail = producerRecord?.email || producerRecord?.effectiveEmail || null;
            const producerEffectiveEmail = producerRecord?.effectiveEmail || producerRecord?.email || null;
            const producerNpn = producerRecord?.npn || null;
            const producerName = producerRecord?.fullName ||
                (producerRecord?.lastName && producerRecord?.firstName
                    ? `${producerRecord.lastName}, ${producerRecord.firstName}`
                    : null);
            const producerPhone = producerRecord?.phone || producerRecord?.cell || null;
            // Pull template producer's signed Carrier-stage appointments
            const templates = await fetch(`https://surelc.surancebay.com/surecrm/appointments-requests?producerId=${templateProducerId}&gaId=${gaId}`, { headers: { Authorization: `Bearer ${bearer}` } }).then((r) => r.json());
            const targets = (Array.isArray(templates) ? templates : []).filter((t) => t.stage === "Carrier" && !existingCarriers.has(t.carrierName));
            const created = [];
            for (const t of targets) {
                ...
                if (!newAppt.producerEmailUsed) {
                    logger.warn({ producerId, carrier: t.carrierName }, "[create-appointment-requests] skipping carrier — producerEmailUsed unresolved");
...
app.post("/recover-orphan-bga-requests", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            logger.info({ producerId, totalAtBga: all.filter((r) => r.stage === "BGA").length, orphans: orphans.length }, "[recover-orphan-bga-requests] discovery");
            const reverted = [];
            const failed = [];
            for (const o of orphans) {
                ...
                try {
                    ...
                    if (r.ok) {
                        ...
                        logger.info({ id: o.appointmentRequestId, carrier: o.carrierName }, "[recover-orphan-bga-requests] reverted BGA → Producer");
                    }
                    else {
    ...
    try {
        ...
        try {
            ...
            while (Date.now() < dl && !bearer)
                await page.waitForTimeout(250);
            if (!bearer)
                return res.status(502).json({ ok: false, error: "no Bearer harvested" });
            // List attachments
            const listRes = await fetch(`https://surelc.surancebay.com/surecrm/attachments/${producerId}?withUndefined=true&withUnlinkedBusinessChecks=false`, { headers: { Authorization: `Bearer ${bearer}` } });
            if (!listRes.ok) {
                const t = await listRes.text().catch(() => "");
                return res.status(502).json({ ok: false, error: `list HTTP ${listRes.status}: ${t.slice(0, 200)}` });
            }
            const attachments = (await listRes.json().catch(() => []));
            ...
            for (const a of candidates) {
                const r = await fetch(`https://surelc.surancebay.com/surecrm/attachments/${a.id}`, { method: "DELETE", headers: { Authorization: `Bearer ${bearer}` } });
...
app.post("/set-producer-fields", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const r = await fetch(`https://surelc.surancebay.com/surecrm/producers/${producerId}/update`, {
                method: "PUT",
                headers: { Authorization: `Bearer ${bearer}`, "Content-Type": "application/json" },
                body,
            });
            ...
            logger.info({ producerId, fields: Object.keys(fields), putStatus: r.status }, "[set-producer-fields] PUT ok");
            return res.json({
                ok: true,
                producerId,
                fieldsSent: Object.keys(fields),
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                if (apiKey) {
                    const cur = await fetch(`https://surelc.surancebay.com/api/v2/producers/${producerId}`, { headers: { "x-api-key": apiKey } });
                    if (cur.ok) {
            ...
            const putRes = await fetch(`https://surelc.surancebay.com/surecrm/producers/${producerId}/update`, {
                method: "PUT",
                headers: { Authorization: `Bearer ${bearer}`, "Content-Type": "application/json" },
            ...
            try {
                const apiKey = process.env.SURELC_API_KEY || process.env.SURELC_API_TOKEN;
                if (apiKey) {
                    const v = await fetch(`https://surelc.surancebay.com/api/v2/producers/${producerId}`, { headers: { "x-api-key": apiKey } });
                    if (v.ok) {
            ...
            logger.info({ producerId, beforeEmail, afterEmail }, "[set-producer-email] success");
            return res.json({
                ok: true,
                producerId,
                beforeEmail,
...
app.post("/patch-appointments-to-resident-state", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (!state)
                return res.status(400).json({ ok: false, error: "no resident state" });
            const patched = [];
...
app.post("/patch-appointment-email", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const patched = [];
            for (const id of appointmentIds) {
                const cur = await fetch(`https://surelc.surancebay.com/surecrm/appointments-requests/${id}`, { headers: { Authorization: `Bearer ${bearer}` } }).then((r) => (r.ok ? r.json() : null));
                if (!cur) {
                ...
                if (!r.ok) {
                    ...
                    const resend = await fetch(`https://surelc.surancebay.com/surecrm/appointments-requests/${id}/email`, {
                        method: "POST",
                        headers: { Authorization: `Bearer ${bearer}` },
                    });
