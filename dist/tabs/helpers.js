...
export function makeTabContext(page, logger, jobId) {
...
export async function inputByLabel(page, labelText) {
    // Strategy 1 — input directly inside the same <label>.
    const inLabel = await page.$(`label:has-text("${labelText}") input`);
    if (inLabel && (await inLabel.isVisible().catch(() => false)))
        return inLabel;
    // Strategy 2 — input referenced by `for=`.
    const forAttr = await page.$$eval("label", (labels, t) => {
    ...
    if (forAttr) {
        ...
        if (ref && (await ref.isVisible().catch(() => false)))
            return ref;
    }
    // Strategy 3 — Material/Angular: input near a mat-label.
    const matLabel = await page.$(`mat-label:has-text("${labelText}") >> xpath=ancestor::*[self::mat-form-field][1] >> input`);
    if (matLabel && (await matLabel.isVisible().catch(() => false)))
        return matLabel;
    return null;
}
 ...
 */
export async function fillIfEmpty(page, labelText, value) {
    const el = await inputByLabel(page, labelText);
    if (!el)
        return "missing";
 ...
 */
export async function fillForce(page, labelText, value) {
    const el = await inputByLabel(page, labelText);
    if (!el)
        return false;
    try {
        ...
        const patched = await page
            ...
            const matchLabel = labels.find((l) => (l.textContent || "")
                .trim()
                .toLowerCase()
                .replace(/\*$/, "")
            ...
            if (ng?.getDirectives) {
                try {
                    const directives = ng.getDirectives(input);
...
export async function selectByLabel(page, labelText, value) {
    ...
    const trigger = await page.$(`mat-label:has-text("${labelText}") >> xpath=ancestor::*[self::mat-form-field][1] >> mat-select`);
 ...
 */
export async function uploadRemoteFile(page, fileInputSelector, remoteUrl, logger) {
    try {
        ...
        const filename = decodeURIComponent(remoteUrl.split("?")[0]?.split("/").pop() || "upload.pdf") ||
            "upload.pdf";
 ...
 */
export async function clickSave(page, candidates = [
 ...
 */
export async function gotoBga(page, targetUrl, logger) {
    ...
    try {
        ...
        if (here === targetPathOnly)
            return { ok: true };
        if (here.startsWith(`${targetPathOnly}/`))
            return { ok: true };
    }
    catch {
    ...
    if (onSameOrigin) {
        try {
            ...
            logger.warn({ here, target: targetUrl }, "gotoBga: pushState did not move router; falling back to hard navigation");
        }
        catch (err) {
            logger.warn({ err: err?.message, target: targetUrl }, "gotoBga: pushState attempt threw; falling back to hard navigation");
        }
    }
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            logger.warn({ attempt, err: err?.message, target: targetUrl }, "gotoBga: hard goto threw; will check URL and retry");
        }
        await page
            .waitForLoadState("networkidle", { timeout: 8_000 })
            .catch(() => { });
        ...
        if (!bounced)
            return { ok: true };
        logger.warn({ attempt, here, target: targetUrl }, "gotoBga: hard goto bounced to OAuth; backing off and retrying");
...
export async function snapshot(ctx, label) {
 ...
 */
export async function isTabComplete(page, doneSelectors) {
