...
async function waitForLogin(page, logger) {
...
async function tryLogin(page, email, password, logger) {
    ...
    if (await loginBtn.count()) {
        try {
            ...
            logger.warn({ err: err.message }, "[bga-tokens] login click threw");
        }
    }
    if (!clicked) {
    ...
    try {
        await page.waitForURL((u) => !/accounts\.surancebay\.com\/oauth\/authorize/i.test(u.href), { timeout: 30_000 });
        ...
        const visibleText = await page
            ...
            if (t.length === 0) {
                document.querySelectorAll("body *").forEach((el) => {
                    if (el.children.length > 0)
                        return;
                    const s = (el.textContent || "").trim();
                    if (s && s.length < 200 && /[a-z]/i.test(s))
                        t.push(s);
                });
            }
            return t.slice(0, 30);
            ...
            .catch(() => []);
        logger.warn({ urlBefore, urlAfter: page.url(), visibleText }, "[bga-tokens] still on /authorize after submit — login likely failed");
        return false;
    }
}
async function waitForTokens(page, logger) {
    const start = Date.now();
    let lastData = {};
    while (Date.now() - start < 60_000) {
        ...
        if (tokens.accessToken) {
            logger.info({ storageKeys: Object.keys(data).length, hasRefresh: !!tokens.refreshToken }, "[bga-tokens] captured tokens");
 ...
 */
function extractTokensFromStorage(data) {
    let accessToken;
    let refreshToken;
