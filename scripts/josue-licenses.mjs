...
console.log("Josue Active licenses:")
for (const l of lic.filter(x => x.status === "Active")) {
  console.log(`  ${l.state} (${l.licenseClass}) ${l.isResidentLicense === "Y" ? "[RESIDENT]" : ""} expires=${l.expires?.slice(0,10)}`)
}
await browser.close()
